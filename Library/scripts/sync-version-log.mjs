...
const COMMIT_OUTPUT_PATH = path.join(process.cwd(), "lib/documentation/commitLog.ts");
...
function renderCommitLog(commits) {
  const serialized = commits
    .map(
      (commit) => `  {\n    hash: ${JSON.stringify(commit.hash)},\n    shortHash: ${JSON.stringify(
        commit.hash.slice(0, 7),
      )},\n    committedAt: ${JSON.stringify(commit.releasedAt)},\n    summary: ${JSON.stringify(
        commit.summary,
      )},\n  }`,
    )
    .join(",\n");
...
function main() {
  ...
  fs.writeFileSync(COMMIT_OUTPUT_PATH, renderCommitLog(commits));

  console.log(`Wrote ${entries.length} version entries to ${path.relative(process.cwd(), OUTPUT_PATH)}`);
  console.log(`Wrote ${commits.length} commit entries to ${path.relative(process.cwd(), COMMIT_OUTPUT_PATH)}`);
