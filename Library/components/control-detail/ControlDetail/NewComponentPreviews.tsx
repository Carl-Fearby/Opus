...
export function InfiniteSelectableListPreview({hasMore=true,selectionIndicator="none",totalItemCount=5000,onAction}:{hasMore?:boolean;selectionIndicator?:"none"|"checkbox"|"radio";totalItemCount?:number;onAction?:(label:string,data:Record<string,unknown>)=>void}){const [items,setItems]=useState(()=>createMailItems(30));const [selectedIds,setSelectedIds]=useState<string[]>([]);return <InfiniteSelectableList ariaLabel="Inbox messages" items={items} height={440} itemHeight={76} totalItemCount={hasMore?totalItemCount:items.length} hasMore={hasMore&&items.length<totalItemCount} selectedIds={selectedIds} selectionIndicator={selectionIndicator} getItemId={(item)=>item.id} onSelectionChange={(ids,context)=>{setSelectedIds(ids);onAction?.(`Selected ${ids.length} message${ids.length===1?"":"s"}`,{selectedIds:ids,selectedMessages:context.selectedItems,focusedId:context.focusedId,reason:context.reason});}} onLoadMore={()=>setItems((current)=>{const next=[...current,...createMailItems(Math.min(20,totalItemCount-current.length),current.length)];onAction?.("Loaded more messages",{loadedCount:next.length,totalItemCount});return next;})} onItemActivate={(item)=>onAction?.(`Opened ${item.subject}`,{message:item})} renderItem={(item)=><article className={styles.mailRow} data-unread={item.unread||undefined}><div className={styles.mailCopy}><strong>{item.sender}</strong><span>{item.subject}</span><small>{item.preview}</small></div><span className={styles.mailMeta}>{item.flagged?<b aria-label="Flagged">◆</b>:null}<time>{item.time}</time></span></article>}/>}
